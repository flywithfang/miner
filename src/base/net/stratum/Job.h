...
namespace xmrig {
	...
	public:
		...
		{
			return isNicehash() ? 0xFFFFFFULL : (nonceSize() == sizeof(uint64_t) ? (-1ULL >> (extraNonce().size() * 4)) : 0xFFFFFFFFULL);
		}
...
#   endif
		...
		{
			return target ? (0xFFFFFFFFFFFFFFFFULL / target) : 0;
		}
...
#   ifdef XMRIG_FEATURE_BENCHMARK
		...
		inline void setBenchSize(uint32_t size)
		{
			m_benchSize = size;
		}
#   endif
